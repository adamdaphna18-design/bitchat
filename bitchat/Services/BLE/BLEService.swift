...
extension BLEService: CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String: Any], rssi RSSI: NSNumber) {
        ...
        // ⚡ Bolt: optimized filter().count to reduce() to prevent O(N) intermediate array allocation
...
extension BLEService {
    ...
    private func updateRSSIThreshold(connectedCount: Int) {
        ...
        // ⚡ Bolt: optimized filter().count to reduce() to prevent O(N) intermediate array allocation
        let recentTimeouts = recentConnectTimeouts.reduce(0) { $0 + (Date().timeIntervalSince($1.value) < TransportConfig.bleRecentTimeoutWindowSeconds ? 1 : 0) }
